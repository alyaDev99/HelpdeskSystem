...
class TechnicianDashboard:
    ...
    def view_available_tickets(self):
        ...
        tree_frame = tk.Frame(self.content_frame, bg="#f7f9fa")
        ...
        x_scroll.config(command=self.available_tree.xview)
        ...
        def refresh_available_tickets():
            ...
            if claimed_count >= 2:
                ...
                # Check if warning label already exists to avoid duplicates
                for widget in self.content_frame.winfo_children():
                    if isinstance(widget, tk.Label) and widget.cget("text") == "You can only claim up to 2 active tickets.":
                        break
                else:
    ...
    def view_my_tickets(self):
        ...
        self.my_tickets_tree = ttk.Treeview(tree_frame, columns=columns, show="headings", height=15,
                                            xscrollcommand=x_scroll.set, yscrollcommand=y_scroll.set)
        x_scroll.config(command=self.my_tickets_tree.xview)
        y_scroll.config(command=self.my_tickets_tree.yview)
        x_scroll.pack(side=tk.BOTTOM, fill=tk.X)
        y_scroll.pack(side=tk.RIGHT, fill=tk.Y)
        self.my_tickets_tree.pack(side=tk.LEFT, fill=tk.BOTH, expand=True)
