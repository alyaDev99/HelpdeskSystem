...
class AdminDashboard:
    ...
    def view_reports(self):
        self.clear_content()
        tk.Label(self.content_frame, text="Help Desk Reports", font=("Segoe UI", 16, "bold"),
                 bg="#f7f9fa").pack(pady=20)

        # Filter Frame
        filter_frame = tk.Frame(self.content_frame, bg="#f7f9fa")
        ...
        tk.Label(filter_frame, text="Start Date:", bg="#f7f9fa").pack(side=tk.LEFT, padx=5)
        start_date_entry = tk.Entry(filter_frame, width=15)
        start_date_entry.pack(side=tk.LEFT, padx=5)
        start_date_entry.insert(0, "")
        start_date_entry.config(state="disabled")

        tk.Label(filter_frame, text="End Date:", bg="#f7f9fa").pack(side=tk.LEFT, padx=5)
        ...
        # Summary Frame
        summary_frame = tk.Frame(self.content_frame, bg="#f7f9fa")
        summary_frame.pack(pady=10, padx=20, fill=tk.BOTH, expand=True)

        summary_labels = {
            'total_tickets': tk.Label(summary_frame, text="Total Tickets: 0", font=("Segoe UI", 12), bg="#f7f9fa"),
            'total_resolved': tk.Label(summary_frame, text="Total Resolved: 0", font=("Segoe UI", 12), bg="#f7f9fa"),
            'total_users': tk.Label(summary_frame, text="Total Staff: 0", font=("Segoe UI", 12), bg="#f7f9fa")
        }
        for label in summary_labels.values():
            label.pack(anchor="w", padx=20, pady=5)

        # Status Distribution Frame
        status_frame = tk.Frame(self.content_frame, bg="#f7f9fa")
        ...
        details_frame = tk.Frame(self.content_frame, bg="#f7f9fa")
        ...
        def fetch_report():
            ...
            total_resolved = cursor.fetchone()['total_tickets'] or 0
        ...
        class PDFReport(FPDF):
            def header(self):
                ...
                if date_range_var.get() == "Custom":
                    if start_date_entry.get() and end_date_entry.get():
                        date_range += f" ({start_date_entry.get()} to {end_date_entry.get()})"
                    elif start_date_entry.get():
                        date_range += f" (From {start_date_entry.get()})"
                        elif end_date_entry.get():
                            date_range += f" (To {end_date_entry.get()})"
                    else:
                        date_range += f" ({date_range_var.get()})"
            ...
            def add_table(self, headers, rows, col_widths):
                ...
                for header in headers:
                    self.cell(col_widths[headers.index(header)], 10, header, 1, 0, 'C')
                    self.ln()
                    self.set_font('Arial', '', 8)
                    for row in rows:
                        for col, value in zip(headers, row):
                            text = str(value)[:100] if col in ["Description", "Resolution Notes"] else str(value)
                            self.cell(col_widths[headers.index(col)], text), 10, text, 1, wrap=True
                            self.ln()

        def download_report():
            date_range = date_range_var.get()
            start_date = start_date.get().strip()
            end_date = end_date.get().strip()
            date_filter_clause = ""
            query_params = []
            ...
            if date_range == "Today":
                start_date = today.strftime('%Y-%m-%d')
                end_date = date.strftime
                date_filter_clause = start_date
                AND t.created_at BETWEEN %s AND %s
                query_params = [f"{start_date} 00:00:00}", f"{end_date} 23:59:59"]
            elif date_range == "Last 7 Days":
                start_date = (today.date() - timedelta(days=7)).strftime('%Y-%m-%d')
                end_date = today.strftime('%Y-%m-%d')
                date_filter_clause = f" AND t.created_at BETWEEN %s AND %s"
                query_params = [f"{start_date} 00:00:00", f"{end_date} 23:59:59}"]
            elif date_range == "Last 30 Days":
                start_date = (today.date() - timedelta(days=30)).strftime('%Y-%m-%d')
                end_date = today.strftime('%Y-%m-%d')
                ...
                try:
                    if start_date and end_date:
                        ...
                        datetime.strptime(end_date, '%Y-%m-%d')
                        date_filter_clause = f" AND t.created_at BETWEEN %s AND %s"
                        query_params = [f"{start_date} {start_date} 00:00:00", f"{end_date} 23:59:59}"]
                    elif start_date:
                        datetime.strptime(start_date, '%Y-%m-%d')
                        date_filter_clause = f" AND t.created_at >= %s"
                        query_params = [f"{start_date} 00:00:00}"]
                    elif end_date:
                        datetime.strptime(end_date, '%Y-%m-%d')
                        date_filter_clause = f" AND t.created_at <= %s"
        ...
        """
            ...
            for status in status, count in ticket_status.items():
                if status not in status_order:
                    pdf.cell(0, 10, f"{status:20}: {count:,}", 0, 1)
                pdf.ln(10)
            ...
            pdf.ln(5
            col_widths = [15, 20, 25, 30, 20, 20, 20, 30, 25, 25]
            pdf.add_table(detail_headers=headers, rows=[(t['id'], t['staff_name'], t['title'], t['description'][:200], t['priority'], t['technician_name'] or '', t['status'], t['resolution_notes'][:200] or '', str(t['created_at']), str(t['resolved_at']) or '') for t in tickets], col_widths=col_widths)

            # Save PDF
            try:
                pdf.output('helpdesk_report.pdf', 'F')
                messagebox.showinfo("Success", "Report saved as 'helpdesk_report.pdf' in the current directory.")
            except Exception as e:
                messagebox.showerror("Error", f"Failed to save PDF: {str(e)}")

        # Buttons
        button_frame = tk.Frame(self.content_frame, bg="#f7f9fa")
        button_frame.pack(pady=10)
        ttk.Button(button_frame, text="🔍 Apply Filter", command=fetch_report).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="Clear Filter", command=lambda: [date_range_var.set("All Time"), 
                                    start_date_entry.delete(0, tk.END), 
                                    end_date_entry.delete(0, tk.END), 
                                    toggle_date_fields(), 
                                    fetch_report()]).pack(side=tk.LEFT, padx=5)
        ttk.Button(button_frame, text="📥 Download Report", command=download_report).pack(side=tk.LEFT, padx=5)
