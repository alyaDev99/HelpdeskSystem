...
class AdminDashboard:
    ...
    def view_reports(self):
        ...
        def generate_report(save_to_file=True):
            ...
            base_widths = [10, 20, 20, 35, 15, 20, 15, 35, 25, 25]
            ...
            for ticket in tickets:
                for header, value in zip(headers, (
                    str(ticket.get('id', '') or '')[:100],
                    str(ticket.get('staff_name', '') or '')[:100],
                    str(ticket.get('title', '') or '')[:100],
                    str(ticket.get('description', '') or '')[:100],
                    str(ticket.get('priority', '') or '')[:100],
                    str(ticket.get('technician_name', '') or '')[:100],
                    str(ticket.get('status', '') or '')[:100],
                    str(ticket.get('resolution_notes', '') or '')[:100],
                    str(ticket.get('created_at', '') or '')[:100],
                    str(ticket.get('resolved_at', '') or '')[:100]
                )):
                    pdf = PDFReport()
