...
def view_reports(self):
    ...
    def fetch_report():
        ...
        for row in tickets:
            clean_row = tuple(
                "" if v is None else str(v)[:100] if col == "description" else str(v)[:200] if col == "resolution_notes" else str(v)
        ...
        return {
            ...
            'tickets': tickets  # Return tickets for generate_report
        }
    ...
    class PDFReport(FPDF):
        def header(self):
            ...
            self.cell(0, 10, 'Help Desk Report - LEVIIT', align='C')
            ...
            self.cell(0, 10, date_range_text, align='C')
            self.ln(10)
        ...
        def add_table(self, headers, rows, col_widths):
            ...
            for row_data in rows:
                row_height = 10
                for col, value, width in zip(headers, row_data, col_widths):
                    text = str(value)[:100] if col in ["Description", "Resolution Notes"] else str(value)
                    self.multi_cell(width, 5, text, border=1, align='L')
                self.ln(row_height)
    ...
    def generate_report(save_to_file=True):
        ...
        pdf.cell(0, 10, f"Total Tickets: {total_tickets:,}", ln=True)
        pdf.cell(0, 10, f"Total Resolved: {total_resolved:,}", ln=True)
        pdf.cell(0, 10, f"Total Staff: {total_users:,}", ln=True)
        pdf.ln(10)

        # Status Distribution Section
        pdf.set_font('Helvetica', 'B', 12)
        pdf.cell(0, 10, 'Ticket Status Distribution', ln=True)
        pdf.set_font('Helvetica', '', 10)
        status_order = ['Pending', 'In Progress', 'Resolved', 'Not Resolved']
        for status in status_order:
            count = ticket_status.get(status, 0)
            pdf.cell(0, 10, f"{status:20}: {count:,}", ln=True)
        for status, count in ticket_status.items():
            if status not in status_order:
                pdf.cell(0, 10, f"{status:20}: {count:,}", ln=True)
        pdf.ln(10)
        ...
        pdf.set_font('Helvetica', 'B', 12)
        pdf.cell(0, 10, 'Detailed Ticket List', ln=True)
        pdf.ln(5)
        ...
        rows = [
            (
                ...
                t.get('priority', ''),
                t.get('technician_name', '') or '',
                t.get('status', ''),
                t.get('resolution_notes', '')[:100] or '',
