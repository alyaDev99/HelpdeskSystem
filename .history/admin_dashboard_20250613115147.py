...
class AdminDashboard:
    ...
    def view_reports(self):
        ...
        # Create a frame for the Ticket Status Distribution Treeview and scrollbars
        ...
        x_scroll_details = ttk.Scrollbar(tree_frame, orient=tk.HORIZONTAL)
        y_scroll_details = ttk.Scrollbar(tree_frame, orient=tk.VERTICAL)
        ...
        detail_tree = ttk.Treeview(tree_frame, columns=detail_columns, show="headings", height=10,
                                   xscrollcommand=x_scroll_details.set, yscrollcommand=y_scroll_details.set)
        x_scroll_details.config(command=detail_tree.xview)
        y_scroll_details.config(command=detail_tree.yview)
        x_scroll_details.pack(side=tk.BOTTOM, fill=tk.X)
        y_scroll_details.pack(side=tk.RIGHT, fill=tk.Y)
        ...
        def fetch_report():
            ...
            sql_tech = f"""
                SELECT u.username AS technician_name, COUNT(t.id) AS count
                FROM users u
                LEFT JOIN tickets t ON u.id = t.technician_id 
                    AND t.status = 'Resolved'{date_filter_clause}
        ...
        class PDFReport(FPDF):
            ...
            def add_table(self, headers, rows):
                ...
                for i in range(len(headers)):
                    ...
                    header_width = self.get_string_width(headers[i]) + 4  # Padding
                    self.set_font('Arial', '', 8)  # Data font
                    max_data_width = max(
                        [self.get_string_width(str(row[i])) + 4 for row in rows] + [header_width]
                    )
                ...
                if total_width > page_width:
                    ...
                    extra_space = (page_width - total_width) / len(col_widths)
        ...
        def generate_report(self, save_to_file=True):
            ...
            for status, count in sorted(ticket_status.items()):
                if status not in status_order:
                    status_rows.append((status, f"{count:,}"))
            pdf.add_table(status_headers, status_rows)
            pdf.ln(10)
            pdf.set_font('Arial', 'B', 12)
            ...
            pdf.add_table(tech_headers, tech_rows)
            pdf.ln(10)
            pdf.set_font('Arial', 'B', 12)
            ...
            pdf_path = 'helpdesk_report.pdf' if save_to_file else 'helpdesk_report.pdf'
            try:
                pdf.output(pdf_path)
                if save_to_file:
        ...
        def print_report(self):
            pdf_path = self.generate_report(save_to_file=False)
            if not pdf_path:
                return
            try:
