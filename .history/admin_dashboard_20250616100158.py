...
class AdminDashboard:
    ...
    for row in rows:
        # Step 1: Calculate the max number of lines for this row
        line_counts = []
        for value, width in zip(row, col_widths):
            value = str(value)
            line_height = 5  # Line height in mm
            num_lines = self.get_string_width(value) / (width - 2)
            lines = int(num_lines) + 1
            line_counts.append(lines)
        max_lines = max(line_counts)
        row_height = max_lines * 5

        # Step 2: Check if we need a new page
        ...
        # Step 3: Draw each cell in the row
        x_start = self.get_x()
        y_start = self.get_y()
        for i, (value, width) in enumerate(zip(row, col_widths)):
            ...
            x = self.get_x()
            y = self.get_y()
            self.multi_cell(width, 5, value, border=1, align='L')
