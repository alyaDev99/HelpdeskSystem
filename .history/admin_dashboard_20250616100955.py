...
class AdminDashboard:
    ...
    def view_reports(self):
        ...
        class PDFReport(FPDF):
            ...
            def get_num_lines_and_lines(self, text, width):
                """
                Manually split `text` into lines that fit within `width` (in mm),
                using current font settings. Returns list of lines.
                """
                if text is None:
                    return [""]
                ...
                for para in paragraphs:
                    ...
                    for word in words[1:]:
                        ...
                        if self.get_string_width(test_line) <= width:
                            ...
                            current = word
                    lines.append(current)
                # Ensure at least one empty line if text was empty
                if not lines:
                    lines = ['']
                return lines
        ...
        line_height = 5  # mm per text line

        for row in rows:
            # 1. For each cell, split into lines that fit column width minus small padding
            cell_lines = []
            line_counts = []
            for value, width in zip(row, col_widths):
                text = str(value) if value is not None else ""
                # Deduct small padding (e.g., 2 mm) so text doesn’t touch borders
                usable_width = width - 2
                lines = self.get_num_lines_and_lines(text, usable_width)
            ...
            # 3. Draw cell by cell, line by line
            y_start = self.get_y()
            x_start = self.get_x()
            for col_idx, (lines, width) in enumerate(zip(cell_lines, col_widths)):
                x = self.get_x()
                y = self.get_y()
                # For each line in this cell, print or blank if fewer lines
                for i in range(max_lines):
                    text_line = lines[i] if i < len(lines) else ""
                    # For the first line in cell, draw top border; for middle lines, draw left/right;
                    # for last line, draw bottom border. But FPDF's cell() border option accepts:
                    # 0: no border, 1: full border, 'L', 'T', etc. To simplify, we draw a full border
                    # around the entire cell by using rect() after printing content, OR:
                    # A simpler method: for each cell, use multi_cell for full block but with padded newline:
                    # However multi_cell resets X; here we manually draw text and then a rectangle.
                    # We will:
                    #  - Draw text line with no border inside cell area.
                    self.set_xy(x, y + i * line_height)
                    # Slight left padding
                    self.cell(width, line_height, txt=text_line, border=0, ln=0, align='L')
                # After printing text lines, draw the cell border rectangle:
                self.rect(x, y_start, width, row_height)
                # Move X to next cell start
                self.set_xy(x + width, y_start)
            # Move Y to next row
        ...
        def generate_report(save_to_file=True):
            ...
            base_widths = [10, 15, 20, 35, 18, 20, 20, 35, 15, 15]
