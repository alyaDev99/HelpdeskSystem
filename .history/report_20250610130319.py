...
class AdminDashboard:
    ...
    def view_reports(self):
        ...
        summary_labels = {
            ...
            'total_users': tk.Label(summary_frame, text="Total Users: 0", font=("Segoe UI", 12), bg="#f7f9fa")
        }
        ...
        def fetch_report():
            ...
            summary_labels['total_users'].config(text=f"Total Users: {total_users:,}")
            ...
            ticket_status = {row['status_lower']: row['count'] for row in cursor.fetchall()}
            
            for status in status_order:
                count = ticket_status.get(status, 0)
                tree.insert("", tk.END, values=(status.replace('_', ' ').title(), f"{count:,}"))
            for status, count in ticket_status.items():
                if status not in status_order:
                    tree.insert("", tk.END, values=(status.title(), f"{count:,}"))
        ...
        class PDFReport(FPDF):
            def header(self):
                ...
                if start_date_entry.get() and end_date_entry.get():
                    ...
                    date_range += f" (Up to {end_date_entry.get()})"
        ...
        def download_report():
            ...
            for status in status_order:
                count = ticket_status.get(status, 0)
                pdf.cell(0, 10, f"{status.replace('_', ' ').title()}: {count:,}", 0, 1)
            for status, count in ticket_status.items():
                if status not in status_order:
            ...
            # Save PDF
            try:
                pdf.output('help_desk_report.pdf', 'F')
                messagebox.showinfo("Success", "Report saved as 'help_desk_report.pdf' in the current directory.")
        ...
        ttk.Button(button_frame, text="❌ Clear Filter", command=lambda: [start_date_entry.delete(0, tk.END), end_date_entry.delete(0, tk.END), fetch_report()]).pack(side=tk.LEFT, padx=5)
