    ...
    def view_reports(self):
        ...
        class PDFReport(FPDF):
            ...
            def add_table(self, headers, rows):
                ...
                page_width = 190  # Page width minus margins (210mm - 10mm - 10mm)
                line_height = 4  # Height per line in mm
                min_width = 10  # Minimum column width in mm
                padding = 2  # Padding for text in mm

                # Calculate max width for each column based on content
                col_widths = [self.get_string_width(header) + padding for header in headers]
                for row in rows:
                    for i, value in enumerate(row):
                        text = str(value)
                        col_widths[i] = max(col_widths[i], self.get_string_width(text) + padding)
                ...
                for row in rows:
                    ...
                    for value, width in zip(row, col_widths):
                        text = str(value)
                        # Estimate lines based on text width
                        lines = max(1, int(self.get_string_width(text) / (width - padding)) + 1)
                        max_lines = max(max_lines, lines)
                    row_height = max_lines * line_height  # Uniform height for row
                    ...
                    for value, width in zip(row, col_widths):
                        text = str(value)
                        self.multi_cell(width, line_height, text, border=1, align='L')
                        self.set_xy(self.get_x() + width, start_y)
