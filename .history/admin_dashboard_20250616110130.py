...
class AdminDashboard:
    ...
    def view_tickets(self):
        ...
        def refresh_tickets():
            ...
            try:
                ...
                # Compute a hash of ticket data to detect changes
                ticket_data = "".join(
                    "".join(str(v) if v is not None else "" for v in row)
                    for row in rows
                )
                current_ticket_hash = hashlib.sha256(ticket_data.encode()).hexdigest()

                # Refresh if ticket count or data has changed
                if current_ticket_count != self.last_ticket_count or current_ticket_hash != self.last_ticket_hash:
                    ...
                    self.last_ticket_hash = current_ticket_hash
    ...
    def view_reports(self):
        ...
        def generate_report(save_to_file=True):
            ...
            base_widths = [10, 15, 20, 30, 20, 25, 20, 40, 10, 10]
