...
class AdminDashboard:
    ...
    def create_dashboard(self):
        ...
        # Configure Treeview style for consistent table appearance
        style = ttk.Style()
        style.configure("Custom.Treeview", font=("Helvetica", 10), rowheight=40, borderwidth=1)
        style.configure("Custom.Treeview.Heading", font=("Helvetica", 10, "bold"), borderwidth=1)
    ...
    def view_users(self):
        ...
        tree = ttk.Treeview(self.content_frame, columns=columns, show="headings", height=15, style="Custom.Treeview")
        for col in columns:
    ...
    def view_tickets(self):
        ...
        self.ticket_tree = ttk.Treeview(tree_frame, columns=columns, show="headings",
                                        xscrollcommand=x_scroll.set, yscrollcommand=y_scroll.set, height=15, style="Custom.Treeview")
        ...
        # Set initial column widths and calculate dynamic widths based on content
        col_widths = {
            "ID": 80, "Staff": 120, "Department": 120, "Title": 200, "Description": 300,
            "Priority": 100, "Technician": 120, "Status": 100, "Resolution Notes": 300,
            "Created At": 150, "Closed At": 150
        }

        for col in columns:
            self.ticket_tree.heading(col, text=col)
            self.ticket_tree.column(col, width=col_widths[col], anchor="w", stretch=True)
        ...
        def refresh_tickets():
            ...
            try:
                ...
                # Calculate maximum text length for Description and Resolution Notes
                max_desc_len = 300  # Minimum width
                max_notes_len = 300
                for row in rows:
                    desc = str(row[4]) if row[4] is not None else ""
                    notes = str(row[8]) if row[8] is not None else ""
                    max_desc_len = max(max_desc_len, len(desc) * 7)  # Approximate pixel width
                    max_notes_len = max(max_notes_len, len(notes) * 7)

                # Update column widths dynamically
                self.ticket_tree.column("Description", width=max_desc_len, stretch=True)
                self.ticket_tree.column("Resolution Notes", width=max_notes_len, stretch=True)
    ...
    def view_reports(self):
        ...
        tech_tree = ttk.Treeview(status_frame, columns=tech_columns, show="headings", height=5, style="Custom.Treeview")
        ...
        tree = ttk.Treeview(status_tree_frame, columns=columns, show="headings", height=5,
                            xscrollcommand=x_scroll.set, yscrollcommand=y_scroll.set, style="Custom.Treeview")
        ...
        # Set initial column widths for detailed ticket list
        detail_col_widths = {
            "ID": 80, "Description": 300, "Priority": 100, "Title": 200, "Staff": 120,
            "Status": 100, "Resolution Notes": 300, "Technician": 120, "Created At": 150, "Closed At": 150
        }

        for col in detail_columns:
            detail_tree.heading(col, text=col)
            detail_tree.column(col, width=detail_col_widths[col], anchor="w", stretch=True)
        ...
        def fetch_report():
            ...
            for ticket in tickets:
                desc = str(ticket['description']) if ticket['description'] is not None else ""
                notes = str(ticket['resolution_notes']) if ticket['resolution_notes'] is not None else ""
                max_desc_len = max(max_desc_len, len(desc) * 7)
                max_notes_len = max(max_notes_len, len(notes) * 7)

            # Update column widths dynamically
            detail_tree.column("Description", width=max_desc_len, stretch=True)
            detail_tree.column("Resolution Notes", width=max_notes_len, stretch=True)
            ...
            def header(self):
                ...
                if date_range_var.get() == "Custom":
                    if start_date_entry.get() and end_date_entry.get():
                        date_range_text += f" ({start_date_entry.get()} to {start_date_entry.get()})"
        ...
        def generate_report(save_to_file=True):
            ...
            for status, count in sorted(ticket_status.items()):
                if status not in status_order:
                    status_rows.append((status, f"{count:,}"))
            pdf.add_table(status_headers, status_rows, status_col_widths)
            pdf.ln(10)
            pdf.set_font('Arial', 'B', 12)
            ...
            pdf.add_table(tech_headers, tech_rows, tech_col_widths)
            pdf.ln(10)
            pdf.set_font('Arial', 'B', 12)
            ...
            col_widths = [10, 20, 20, 35, 15, 20, 15, 35, 25, 25]
            ...
            if save_to_file:
                ...
                # Use a temporary file for printing
