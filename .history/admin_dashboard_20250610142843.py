...
class AdminDashboard:
    ...
    def view_reports(self):
            ...
            def add_table(self, headers, rows, col_widths):
                ...
                for row in rows:
                    ...
                    for col, value, width in zip(headers, row, col_widths):
                        ...
                        lines = self.get_string_width(text) / (width - 2) + 1  # Estimate lines, account for padding
                        cell_height = int(lines) * 4  # Approximate height per line
                        max_height = max(max_height, cell_height)
                    
                    # Draw each cell in the row
                    for col, value, width in zip(headers, row, col_widths):
                        text = str(value)[:100] if col in ["Description", "Resolution Notes"] else str(value)
                        self.multi_cell(width, max_height / (self.get_string_width(text) / (width - 2) + 1), 
                                      text, 1, 'L', fill=False)
                    self.ln(max_height)
        ...
        def generate_report(save_to_file=True):
            ...
            col_widths = [10, 20, 20, 35, 15, 20, 15, 35, 25, 25]  # Sum = 220, scaled down to fit
