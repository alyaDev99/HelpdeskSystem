...
class StaffDashboard:
    def __init__(self, user_id):
        ...
        self.last_tickets = []  # Track ticket data for updates
    ...
    def show_view_tickets(self):
        ...
        def refresh_tickets():
            ...
            try:
                ...
                # Convert tickets to a list of tuples with cleaned values for comparison
                current_tickets = [tuple("" if val is None else str(val) for val in ticket) for ticket in tickets]

                # Compare current tickets with last tickets
                if current_tickets != self.last_tickets:
                    for item in tree.get_children():
                        tree.delete(item)
                    for ticket in tickets:
                        clean_ticket = tuple("" if val is None else str(val) for val in ticket)
...
if __name__ == "__main__":
    StaffDashboard(user_id=1)  # Example user_id